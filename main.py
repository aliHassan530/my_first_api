...
# Cloudinary config (use .env in production)
cloudinary.config(
    cloud_name="puzzleApp",
    api_key="665944475181268",
    api_secret="HAbYE1y7-wP1BOcB6aXrsey-Q7M",
    secure=True
)
...
def signup(name: str = Body(...), email: str = Body(...), password: str = Body(...)):
    ...
    return {"status": "success", "message": "User registered successfully!", "user": {"name": name, "email": email}}
...
def mark_attendance(email: str = Body(...)):
    ...
    record = {"email": email, "name": user["name"], "time": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
...
def getallPost():
    ...
    return {"total_posts": len(posts), "posts": [serialize_doc(p) for p in posts]}
...
async def upload_image(file: UploadFile = File(...)):
    try:
        ...
        # Save to MongoDB
        images_collection.insert_one({"url": image_url, "uploaded_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")})

        return {"status": "success", "message": "Image uploaded and saved to MongoDB!", "url": image_url}
